...
'''LRS module that takes conv(s) as input, and does relu(conv(s)) concatenated with tanh(conv(s)**2)'''
def convlrs(inp):
    term_1 = tf.nn.relu(inp)
    term_2 = tf.nn.tanh(tf.square(inp))
    out = tf.concat([term_1, term_2], axis=-1)
    return out
